...
echo 'type song name 	=	It will play in vlc'
else
...
then
wget $($CURLS "$getting_music_website" | grep '<div class="player" data-play="1"' | grep -o "http.*.mp3")
else
wget $($CURLS http://mp3miner.com/$getting_music_website | grep 'song-audio' | grep -o "http.*preload" | sed 's/" preload//g')
fi
fi
